...
export async function sendAssessmentInvitation(input: SendAssessmentInvitationInput) {
  ...
  return recordAndSendCommunication({
    ...
    eventKey: "ASSESSMENT_INVITATION_SENT",
    sendType: "MANUAL",
    idempotencyKey: `assessment-invitation:${input.prospectId}:${input.campaignId ?? "direct"}`,
    triggeredBy: "Assessment invitation action",
    relatedEntityType: "Prospect",
    relatedEntityId: input.prospectId,
